...
- **bos-only (`requireBos`)**: create branch (`POST /branches`), delete branch (`DELETE /branches/:id`), settings (`PATCH /settings`), salary-slip writes (`POST|PATCH|DELETE /salary-slips`), user management (`POST|PATCH|DELETE /users`), add/delete bank accounts (`POST|DELETE /banks`), delete branch deposit (`DELETE /branches/:id/deposits/:depId`), delete voucher recap (`DELETE /voucher-recaps/:id`).
