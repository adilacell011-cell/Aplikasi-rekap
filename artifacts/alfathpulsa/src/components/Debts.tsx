      ...
      <div className="flex flex-col min-h-full bg-transparent pb-40">
        <header className="glass-card px-4 py-4 flex items-center gap-4 shadow-xl border-b border-white/10 sticky top-0 z-20">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] font-black text-white/60 uppercase tracking-widest">Detail Piutang</p>
          </div>
        </header>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
          <div className="glass-card p-5 rounded-2xl shadow-2xl border border-white/10 text-center relative overflow-hidden group h-full flex flex-col justify-center">
            ...
            <p className="text-[10px] font-black text-white/60 uppercase tracking-[0.2em] mb-3 relative z-10">Total Hutang</p>
                ...
                <form onSubmit={handleAddDetail} className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="0"
                      inputMode="numeric"
                      className="w-full pl-10 pr-4 py-3.5 text-sm glass-card border border-white/10 rounded-2xl focus:ring-2 focus:ring-brand-500 outline-none text-white font-black"
                  ...
                  <div className="flex gap-3">
                    <input
                      ...
                      className="flex-1 px-4 py-3.5 text-sm glass-card border border-white/10 rounded-2xl focus:ring-2 focus:ring-brand-500 outline-none text-white font-medium"
                      value={descInput}
            ...
            <div className="divide-y divide-asphalt-700/50">
              {selectedPerson.details.length === 0 ? (
                <div className="py-16 flex flex-col items-center justify-center opacity-20">
                  <HistoryIcon className="w-12 h-12 text-white/60 mb-2" />
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/60">Belum ada aktivitas</p>
                </div>
              ) : (
                [...selectedPerson.details].reverse().map((detail) => (
                  <div key={detail.id} className="p-3.5 flex items-center gap-3 hover:bg-white/10 transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[8px] font-bold text-white/60 uppercase tracking-widest mt-0.5">{formatDate(detail.date)}</p>
                    </div>
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Status Piutang</p>
          ...
          <div className="pt-6 border-t border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex -space-x-3">
                {[...Array(Math.min(3, filteredDebts.length))].map((_, i) => (
                  <div key={i} className="w-8 h-8 rounded-xl border-2 border-white/10 bg-white/10 flex items-center justify-center">
            ...
            <button className="bg-white/10 text-white/60 hover:text-white p-2 rounded-xl border border-white/10 transition-all">
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xs font-black text-white/90 uppercase tracking-[0.2em]">Daftar Pelanggan</h3>
          </div>
          <BlockChoice
            size="sm"
            value={sortBy}
          ...
          <div className="p-5 bg-white/5 border-b border-white/10 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-[9px] font-black transition-all uppercase tracking-wider border whitespace-nowrap ${selectedBranchId === 'all' ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20 border-brand-500' : 'glass-card border-white/10 text-white/60 hover:border-white/10'}`}
                  >
                    Global
                  </button>
                  {store.branches.map(b => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-[9px] font-black transition-all uppercase tracking-wider border whitespace-nowrap ${selectedBranchId === b.id ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20 border-brand-500' : 'glass-card border-white/10 text-white/60 hover:border-white/10'}`}
                    >
                      {b.name}
                    </button>
                  ))}
              ...
              <form onSubmit={handleAddPerson} className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-3.5 text-xs glass-card border border-white/10 rounded-2xl focus:ring-2 focus:ring-brand-500 outline-none text-white font-black placeholder:text-white/60/50"
                    value={newPersonName}
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3.5 text-sm glass-card border border-white/10 rounded-2xl focus:ring-2 focus:ring-rose-500 outline-none text-white font-medium placeholder:text-white/60/50"
                value={searchQuery}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 divide-y sm:divide-y-0 divide-asphalt-700/50">
            {filteredDebts.length === 0 ? (
              <div className="col-span-full text-center py-16 glass-card/50">
                ...
                <p className="text-[10px] font-black text-white/60 uppercase tracking-widest">Tidak ada data pelanggan</p>
              ...
              filteredDebts.map((person) => {
                ...
                return (
                  <div key={person.id} className="flex items-center p-3.5 hover:bg-white/10 transition-all group active:bg-white/10">
                    <div 
                    ...
                    <div className="flex items-center gap-2.5 shrink-0 ml-1">
                      ...
                      {canDelete && (
                        <button
                          ...
                          className="p-1.5 text-white/60 hover:text-rose-500 bg-white/10 hover:bg-white/10 rounded-lg transition-all"
                        >
